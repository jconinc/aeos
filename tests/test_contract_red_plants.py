...
def registered() -> RegisteredOperation:
    return RegisteredOperation(
        ...
        frozenset({"article_id", "expected_digest", "draft"}),
        frozenset({"content_revision"}),
    )


def ready_context() -> AuthorizationContext:
    return AuthorizationContext("1", "a" * 64, True, True, 0)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda: EffectTemplate("op", {}, (), "post", True, cost_ceiling_minor_units=-1),
        lambda: ModelCallIdentity("provider", "model", "bad", "f" * 64, 1),
        lambda: ModelCallIdentity("provider", "model", "e" * 64, "f" * 64, 0),
        lambda: ModelDecision(
            "choice",
            "reason",
            ("e1",),
            1.1,
            ModelCallIdentity("provider", "model", "e" * 64, "f" * 64, 1),
            {},
        ),
...
def test_effect_authorization_refusal_surface_is_closed() -> None:
    ...
    cases = (
        ...
        (
            ...
            human,
            registered(),
            replace(ready_context(), effects_enabled=False),
            RefusalCode.EFFECT_PRECONDITION_FAILED,
        ),
        (recommendation, selected, human, None, ready_context(), RefusalCode.EFFECT_NOT_REGISTERED),
        (
            recommendation,
            selected,
            human,
            replace(registered(), boundary_tags=frozenset({"publish"})),
            ready_context(),
...
def test_model_policy_and_response_red_plants_fail_closed() -> None:
    ...
    no_calls = replace(model_policy, max_model_calls=0)
    budget = DecisionEngine(
        verifier=AcceptingVerifier(),
        clock=FixedClock(),
        model_gateway=ScriptedModel(["candidate-1"]),
    ).decide(packet(authority_policy=no_calls), candidates)
