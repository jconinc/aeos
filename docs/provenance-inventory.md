...
| Mailbox management (`docs/mailbox-management-spec.md` v5 §5, §8; packet 6) | The registry sync's routed rows, the mailbox registry reader, the existing owned-action queue and its founder/operator split | `wema.mail_triage@1` (`adapters/wema_mail.py`, 0.6.0) maps a routed row to a packet and closed candidates; Wema runs the engine, registers the two outward operations, deals the card and attests every send. |
